...
CPU timers run only on their private turns, never display their hands, stop when hidden or leaving play, and resume when visible. With autoplay enabled, shared reveal and results advance only after every human has finished the hole. Hole scorecards always require a click. CPU turns run independently of the autoplay preference. Work is bounded by eight cards and four targets, with cached opponent estimates per action/target; no background simulation, API or worker is needed.
